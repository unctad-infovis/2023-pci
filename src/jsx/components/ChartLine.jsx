...
function LineChart({
  allow_decimals = false, data, idx, line_width = 3, note = false, show_first_label = true, source, subtitle = false, xlabel, title
}) {
  ...
  const createChart = useCallback(() => {
    Highcharts.chart(`chartIdx${idx}`, {
      ...
      colors: ['#009edb', '#72bf44', '#f58220', '#eb1847', '#a05fb4'],
      credits: {
        enabled: false
      },
      exporting: {
      ...
      tooltip: {
        ...
        formatter() {
          ...
          const rows = [];
          rows.push(values.map((point) => `<div><span class="tooltip_label">${(point[0]) ? `${point[0]}: ` : ''}</span> <span class="tooltip_value">${roundNr(point[1], 1).toLocaleString('en-US')}</span></div>`).join(''));
          // eslint-disable-next-line react/no-this-in-sfc
  ...
  }, [allow_decimals, data, idx, line_width, note, show_first_label, source, subtitle, xlabel, title]);
